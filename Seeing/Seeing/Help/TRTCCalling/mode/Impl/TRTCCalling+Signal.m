    ...
    case CallAction_Call:
        {
            ...
            if (isGroup) {
                @weakify(self)
                callID = [[V2TIMManager sharedInstance] inviteInGroup:realModel.groupid inviteeList:realModel.invitedList data:data onlineUserOnly:YES timeout:SIGNALING_EXTRA_KEY_TIME_OUT succ:^{
                    @strongify(self)
                ...
                @weakify(self)
                callID = [[V2TIMManager sharedInstance] invite:realModel.invitedList.firstObject data:data onlineUserOnly:YES offlinePushInfo:[[V2TIMOfflinePushInfo alloc] init] timeout:SIGNALING_EXTRA_KEY_TIME_OUT succ:^{
                    @strongify(self)
                    // 发起 Apns 推送
    ...
    case CallAction_End:
        {
            if (isGroup) {
                ...
                callID = [[V2TIMManager sharedInstance] inviteInGroup:realModel.groupid inviteeList:@[@"inviteeList"] data:data onlineUserOnly:YES timeout:0 succ:nil fail:^(int code, NSString *desc) {
                    if ([self canDelegateRespondMethod:@selector(onError:msg:)]) {
                        [self.delegate onError:code msg:desc];
                    };
                }];
                ...
                if (self.startCallTS > 0) {
                    ...
                    NSString *data = [TRTCCallingUtils dictionary2JsonStr:param];
                    callID = [[V2TIMManager sharedInstance] invite:receiver data:data onlineUserOnly:true offlinePushInfo:[[V2TIMOfflinePushInfo alloc] init] timeout:0 succ:nil fail:^(int code, NSString *desc) {
                        if ([self canDelegateRespondMethod:@selector(onError:msg:)]) {
                            [self.delegate onError:code msg:desc];
                        };
                    }];
