    ...
    func updateLayout() {
        ...
        if collectionCount == 2 {
            ...
            if collectionCount == 1 {
                setLocalViewInVCView(frame: view.window?.bounds ?? CGRect.zero)
            }
        }
    }
}
...
extension TRTCCallingVideoViewController {
    func setupUI() {
        ...
        localPreView.frame = view.window?.bounds ?? CGRect.zero
    ...
    func setupSponsorPanel(topPadding: CGFloat) {
        ...
        if let sponsor = curSponsor {
            ...
            userImage.kf.setImage(with: URL(string: sponsor.avatarUrl))
